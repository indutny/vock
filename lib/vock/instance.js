...
function Instance(options) {
  ...
  // Create audio unit
  this.audio = vock.audio.create(this.options.rate || 24000);
  this.audio.start();
...
Instance.prototype.init = function init() {
  ...
  this.socket.on('data', function(packet, rinfo) {
    ...
    self.getPeer(id, rinfo).receive(packet);
  });
...
Instance.prototype.getPeer = function getPeer(id, info) {
...
Instance.prototype.connect = function connect(id, port, address, callback) {
  var self = this,
      peer = this.getPeer(address + '#' + port, {
        port: port,
        address: address
      });
  ...
  peer.connect(id, function() {
    peer.removeListener('connectTimeout', onConnectTimeout);
    callback && callback.call(self);
  });

  // On connection timeout switch to relay mode
  peer.once('connectTimeout', onConnectTimeout);
