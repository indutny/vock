...
function Protocol() {
  ...
  this.pripub = pripub.create({ password: process.env.VOCK_KEY_PASSWORD });
...
Protocol.prototype.receive = function receive(packet) {
  ...
  if (packet.enc) {
    ...
    var decipher = crypto.createDecipher('aes256', 'hakamada'),
        dec = decipher.update(packet.enc.toString('hex'), 'hex', 'utf-8') +
...
Protocol.prototype.handleHello = function handleHello(packet) {
  ...
  this.emit('authorize', fingerprint, function(ok) {
    ...
    if (self.state !== 'accepted') self.connect();
  });
};
