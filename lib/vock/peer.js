...
Peer.prototype.init = function init() {
  ...
  function resetTimeout() {
    if (self.timers.death) clearTimeout(self.timers.death);
    self.timers.death = setTimeout(function() {
      self.reset();
    }, self.intervals.global);
  };

  this.on('connect', function() {
    if (self.timers.ping) clearInterval(self.timers.ping);
...
Peer.prototype.handleHello = function handleHello(packet) {
  ...
  if (this.timers.wait) clearInterval(this.timers.wait);
  this.timers.wait = setInterval(function() {
  ...
  this.emit('authorize', fingerprint, function(ok) {
    clearInterval(self.timers.wait);
...
Peer.prototype.connect = function connect(id, callback) {
  ...
  this.timers.handshake = setInterval(send, this.intervals.handshake);
  send();
  ...
  this.timers.connect = setTimeout(function() {
    ...
    clearInterval(self.timers.handshake);
